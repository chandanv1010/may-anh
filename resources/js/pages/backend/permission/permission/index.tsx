...
const pageConfig: PageConfig<Permission> = {
    ...
    cardHeading: 'Bảng quản lý danh sách Quyền hệ thống',
    cardDescription: 'Quản lý thông tin danh sách Quyền hệ thống, sử dụng các chức năng để lọc dữ liệu vv...',
    filters: [...filter],
    columns: [
        { key: 'checkbox', label: '', className: 'w-[60px]' },
        { key: 'id', label: 'ID', className: 'w-[60px]' },
        { key: 'name', label: 'Tên nhóm', className: 'w-[15%]' },
        { key: 'canonical', label: 'Tên nhóm', className: 'w-[10%]' },
...
export default function PermissionIndex({users, records}: IPermissionIndexProps) {
    ...
    const {
        ...
        handleCheckItem,
        setSelectedIds
     } = useTable<Permission>({pageConfig, records: records.data})
