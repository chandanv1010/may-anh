...
const pageConfig: PageConfig<ProductBrand> = {
    ...
    cardHeading: 'Bảng quản lý danh sách Thương Hiệu',
    cardDescription: 'Quản lý thông tin danh sách Thương Hiệu, sử dụng các chức năng để lọc dữ liệu vv...',
    filters: [...filter],
    columns: [
        { key: 'checkbox', label: '', className: 'w-[60px] text-center' },
        { key: 'id', label: 'ID', className: 'w-[40px] text-center' },
        { key: 'name', label: 'Tên thương hiệu', className: 'w-[25%]' },
...
const TableRowComponent = React.memo(({
    ...
    return (
        <TableRow key={item.id} className={`cursor-pointer ${checked ? 'bg-[#ffc]' : ''}`}>
            ...
            <TableCell>
                {item.current_language?.name || item.name || 'N/A'}
            </TableCell>
            ...
            <TableCell className="text-center">
                <CustomLanguageFlags 
                    ...
                    postId={item.id}
                    module={pageConfig.module || 'product_brand'}
                    variant="table"
                />
            </TableCell>
...
interface IProductBrandIndexProps {
    users: User[],
    records: IPaginate<ProductBrand>,
    languages?: Array<{id: number, name: string, canonical: string, image: string}>
}
export default function ProductBrandIndex({users, records, languages = []}: IProductBrandIndexProps) {
    ...
    const {
        ...
        robotsLoading,
        robotsValues
     } = useTable<ProductBrand>({pageConfig, records: records.data})
