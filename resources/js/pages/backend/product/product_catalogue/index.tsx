...
import { ProductCatalogue } from './save';
...
const pageConfig: PageConfig<ProductCatalogue> = {
    module: 'product_catalogue',
    heading: 'Quản lý Nhóm Sản Phẩm',
    cardHeading: 'Bảng quản lý danh sách Nhóm Sản Phẩm',
    cardDescription: 'Quản lý thông tin danh sách Nhóm Sản Phẩm, sử dụng các chức năng để lọc dữ liệu vv...',
    filters: [...filter],
    columns: [
    ...
    return (
        <TableRow key={item.id} className={`cursor-pointer ${checked ? 'bg-[#ffc]' : ''}`}>
            ...
            <TableCell>
                ...
                {item.current_language?.name || 'N/A'}
            </TableCell>
...
interface IProductCatalogueIndexProps {
    users: User[],
    records: IPaginate<ProductCatalogue>,
    languages?: Array<{id: number, name: string, canonical: string, image: string}>
}
export default function ProductCatalogueIndex({users, records, languages = []}: IProductCatalogueIndexProps) {
    ...
    const {
        ...
        robotsLoading,
        robotsValues
     } = useTable<ProductCatalogue>({pageConfig, records: records.data})
