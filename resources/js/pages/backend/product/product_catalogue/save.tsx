...
import product_catalogue from '@/routes/product_catalogue';
...
export interface ProductCatalogue extends IDateTime {
...
interface ProductCatalogueSaveProps {
    record?: ProductCatalogue,
    catalogues: TParentCatalogue
}
export default function ProductCatalogueSave({ record, catalogues }: ProductCatalogueSaveProps) {

    const {
        initData
    } = usePageSetup<ProductCatalogue>({ record })

    const formInitData = useMemo(() => ({
        ...initData,
        parent_id: record?.parent_id ? record.parent_id : 0
    }), [initData, record])
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl page-wrapper">
                ...
                <div className="page-container">
                    <FormProvider initData={formInitData}>
                        <Form
                            options={{
                                preserveScroll: true,
                            ...
                            action={
                                isEdit ? product_catalogue.update(record.id) : product_catalogue.store()
                            }
                            method="post"
