...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Thêm mới chương trình Mua X Tặng Y', href: '/' },
]
...
export default function BuyXGetY({ 
    ...
    // Buy X states
    const [buyMinQuantity, setBuyMinQuantity] = useState<number>(promotion?.buy_min_quantity || 1)
    const [buyConditionType, setBuyConditionType] = useState<'min_quantity' | 'min_order_value'>(
        promotion?.buy_condition_type || 'min_quantity'
    )
    const [buyMinOrderValue, setBuyMinOrderValue] = useState<number>(promotion?.buy_min_order_value || 0)
    ...
    const [buySelectedCatalogues, setBuySelectedCatalogues] = useState<Array<{ id: number; name: string; image?: string }>>(
        promotion?.buy_product_catalogue_items || []
    )
    ...
    // Get Y states
    const [getQuantity, setGetQuantity] = useState<number>(promotion?.get_quantity || 1)
    ...
    const [getSelectedCatalogues, setGetSelectedCatalogues] = useState<Array<{ id: number; name: string; image?: string }>>(
        promotion?.get_product_catalogue_items || []
    )
    ...
    const [maxApplyPerOrder, setMaxApplyPerOrder] = useState<number | undefined>(promotion?.max_apply_per_order)
    const [hasMaxApplyLimit, setHasMaxApplyLimit] = useState<boolean>(!!promotion?.max_apply_per_order)
    ...
    // Sync states when promotion changes
    useEffect(() => {
        if (promotion?.buy_product_items && Array.isArray(promotion.buy_product_items)) {
            const validItems = promotion.buy_product_items.filter(item => item && item.id && item.name)
    ...
    useEffect(() => {
        if (promotion?.buy_product_catalogue_items && Array.isArray(promotion.buy_product_catalogue_items)) {
            const validItems = promotion.buy_product_catalogue_items.filter(item => item && item.id && item.name)
            if (validItems.length > 0) {
    ...
    useEffect(() => {
        if (promotion?.get_product_items && Array.isArray(promotion.get_product_items)) {
            const validItems = promotion.get_product_items.filter(item => item && item.id && item.name)
            if (validItems.length > 0) {
    ...
    useEffect(() => {
        if (promotion?.get_product_catalogue_items && Array.isArray(promotion.get_product_catalogue_items)) {
            const validItems = promotion.get_product_catalogue_items.filter(item => item && item.id && item.name)
            if (validItems.length > 0) {
    ...
    const getBuyConditionText = (): string => {
        if (buyConditionType === 'min_quantity') {
            const qty = buyMinQuantity > 0 ? buyMinQuantity : (promotion?.buy_min_quantity || 1)
            return `Mua tối thiểu ${qty} sản phẩm`
        } else {
            const value = buyMinOrderValue > 0 ? buyMinOrderValue : (promotion?.buy_min_order_value || 0)
    ...
    const getGetApplyText = (): string => {
        const qty = getQuantity > 0 ? getQuantity : (promotion?.get_quantity || 1)
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={isEdit ? 'Cập nhật chương trình Mua X Tặng Y' : 'Thêm mới chương trình Mua X Tặng Y'} />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl page-wrapper">
                <CustomPageHeading 
                    heading={isEdit ? 'Cập nhật chương trình Mua X Tặng Y' : 'Thêm mới chương trình Mua X Tặng Y'}
                            ...
                            <div className="max-w-[1400px] ml-auto mr-auto">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    ...
                                    <div className="lg:col-span-2 space-y-6">
                                        {/* Thông tin chung */}
                                        <CustomCard isShowHeader={true} title="Chương trình khuyến mại">
                                            <div className="space-y-2">
                                                <Label htmlFor="name">
                                                    Tên chương trình khuyến mại <span className="text-red-500">*</span>
                                                </Label>
                                                <Input
                                                    id="name"
                                                    name="name"
                                            ...
                                            <div className="space-y-6">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="space-y-4">
                                                        ...
                                                        <div>
                                                            <Label className="mb-4 block font-semibold">Áp dụng cho</Label>
                                    ...
                                    <div className="lg:col-span-1 space-y-6">
                                        ...
                                        <PromotionOverview
                                            ...
                                            items={[
                                                { label: 'Loại khuyến mại', value: <strong>Mua X Tặng Y</strong> },
                                                ...(buyMinQuantity > 0 || (isEdit && promotion?.buy_min_quantity) ? [{ label: 'Mua X', value: getBuyConditionText() }] : []),
                                                { label: 'Áp dụng cho Mua X', value: getBuyApplyText() },
                                                ...(getQuantity > 0 || (isEdit && promotion?.get_quantity) ? [{ label: 'Tặng Y', value: getGetApplyText() }] : []),
                                                { 
                                                    label: 'Loại giảm giá', 
                                                    value: (discountType !== 'free' && discountValue > 0) || (isEdit && promotion?.discount_type !== 'free' && promotion?.discount_value) 
                                                        ? getDiscountText() 
                                                        : 'Miễn phí' 
                                                },
                                                { 
