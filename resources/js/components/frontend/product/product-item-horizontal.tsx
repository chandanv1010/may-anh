...
/**
 * ProductItemHorizontal Component
 * 
 ...
 */
export default function ProductItemHorizontal({ product, className }: { product: Product, className?: string }) {
    const formatCurrency = (amount: number) => {
        return new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(amount);
    };

    return (
        <div className={cn("group flex flex-col md:flex-row bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all duration-300", className)}>
            {/* Image Section - Left */}
