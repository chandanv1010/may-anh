...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
	(
		...
		const prevDefaultValueRef = React.useRef<string[]>([]);
		
		// Initialize với defaultValue ban đầu
		if (prevDefaultValueRef.current.length === 0) {
		...
		// Sync selectedValues với defaultValue khi defaultValue thay đổi
		// Luôn sync để đảm bảo MultiSelect reflect đúng giá trị từ props
		React.useEffect(() => {
			const prevDefaultValue = prevDefaultValueRef.current;
			// So sánh bằng cách sort và stringify để tránh vấn đề với reference
			const prevStr = JSON.stringify([...prevDefaultValue].sort());
			const currentStr = JSON.stringify([...defaultValue].sort());
			const defaultValueChanged = prevStr !== currentStr;
			
			if (defaultValueChanged) {
				// Luôn sync khi defaultValue thay đổi
				// Tạo copy mới để đảm bảo reference thay đổi
				const newDefaultValue = [...defaultValue];
				setSelectedValues(newDefaultValue);
				prevDefaultValueRef.current = newDefaultValue;
			}
		}, [defaultValue]);
				...
				<Popover
					...
					<PopoverTrigger asChild>
						<Button
							...
							{selectedValues.length > 0 && responsiveSettings.maxCount > 0 ? (
								...
								<div className="flex items-center justify-between w-full mx-auto">
									<span className="text-muted-foreground mx-3 font-normal" style={{ fontSize: '13px' }}>
										{selectedValues.length > 0 && responsiveSettings.maxCount === 0 
