    ...
    public function run(): void
    {
        ...
        try {
            ...
            if ($userCatalogues->isEmpty()) {
                ...
                if ($assignedCount > 0) {
                    $this->command->info("Đã gán {$assignedCount} supplier permissions cho " . $userCatalogues->count() . " user catalogue(s).");
                } else {
                    $this->command->info('Tất cả user catalogues đã có supplier permissions.');
                }
            }

            DB::commit();
            $this->command->info('✅ Đã tạo permissions cho Supplier module thành công!');
        } catch (\Throwable $th) {
            DB::rollBack();
