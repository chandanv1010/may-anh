...
class CustomerPermissionSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $modules = [
            [
                'snake_module' => 'customer_catalogue',
                'display_name' => 'Nhóm Khách Hàng',
            ],
            [
                'snake_module' => 'customer',
        ...
        try {
            ...
            if ($userCatalogues->isEmpty()) {
                ...
                if ($assignedCount > 0) {
                    $this->command->info("Đã gán {$assignedCount} customer permissions cho " . $userCatalogues->count() . " user catalogue(s).");
                } else {
                    $this->command->info('Tất cả user catalogues đã có customer permissions.');
                }
            }

            DB::commit();
            $this->command->info('✅ Đã tạo permissions cho Customer modules thành công!');
        } catch (\Throwable $th) {
            DB::rollBack();
