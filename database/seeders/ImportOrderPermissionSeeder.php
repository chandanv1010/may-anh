...
class ImportOrderPermissionSeeder extends Seeder
{
    /**
        ...
        try {
            ...
            $permissionIds = DB::table('permissions')
                ->whereIn('canonical', [
                    ...
                    'import_order:update',
                    'import_order:delete',
                    'import_order:bulkDestroy',
                    'import_order:bulkUpdate',
                ])
                ->pluck('id')
                ->toArray();
            ...
            if ($userCatalogues->isEmpty()) {
                ...
                if ($assignedCount > 0) {
                    $this->command->info("Đã gán {$assignedCount} import_order permissions cho " . $userCatalogues->count() . " user catalogue(s).");
                } else {
                    $this->command->info('Tất cả user catalogues đã có import_order permissions.');
                }
            }

            DB::commit();
            $this->command->info('✅ Đã tạo permissions cho ImportOrder module thành công!');
        } catch (\Throwable $th) {
            DB::rollBack();
