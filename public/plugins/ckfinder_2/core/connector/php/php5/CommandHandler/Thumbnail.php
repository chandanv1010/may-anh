...
class CKFinder_Connector_CommandHandler_Thumbnail extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
        ...
        // Check if file is SVG - this format cannot be thumbnailed by PHP GD
        // WebP may be supported if PHP GD was compiled with WebP support
        $fileExtension = strtolower(pathinfo($fileName, PATHINFO_EXTENSION));
        $isUnsupportedFormat = ($fileExtension === 'svg');
        $isWebP = ($fileExtension === 'webp');
        $webpSupported = function_exists('imagecreatefromwebp') && (imagetypes() & IMG_WEBP);
        ...
        if (!file_exists($thumbFilePath)) {
            // For SVG, copy the original file as thumbnail (browser will render it)
            // For WebP, try to create thumbnail if PHP supports it, otherwise copy original
            if ($isUnsupportedFormat) {
                $thumbDir = dirname($thumbFilePath);
                if (!file_exists($thumbDir)) {
                    mkdir($thumbDir, 0755, true);
                }
                copy($sourceFilePath, $thumbFilePath);
            } else {
            if(!$this->createThumb($sourceFilePath, $thumbFilePath, $_thumbnails->getMaxWidth(), $_thumbnails->getMaxHeight(), $_thumbnails->getQuality(), true, $_thumbnails->getBmpSupported())) {
                    // If thumbnail creation fails (e.g., WebP not supported), try to use original file
        ...
        // For SVG, use original file's mime type
        // For WebP, try to get from thumbnail, fallback to original if needed
        if ($isUnsupportedFormat) {
            $sourceImageAttr = @getimagesize($sourceFilePath);
            if ($sourceImageAttr === false) {
                $mime = 'image/svg+xml'; // SVG fallback
            } else {
                $mime = $sourceImageAttr["mime"];
            }
        } else {
            $sourceImageAttr = @getimagesize($thumbFilePath);
            if ($sourceImageAttr === false && $isWebP) {
                // WebP thumbnail might not exist if PHP doesn't support it
                $sourceImageAttr = @getimagesize($sourceFilePath);
                $mime = ($sourceImageAttr !== false) ? $sourceImageAttr["mime"] : 'image/webp';
            } else {
        $mime = $sourceImageAttr["mime"];
            }
        }
    ...
    public static function createThumb($sourceFile, $targetFile, $maxWidth, $maxHeight, $quality, $preserverAspectRatio, $bmpSupported = false)
    {
        ...
        switch ($sourceImageAttr['mime'])
        {
            ...
            case 'image/webp':
                {
                    // WebP support requires PHP GD compiled with WebP support (PHP 7.0+)
                    if (function_exists('imagecreatefromwebp') && (imagetypes() & IMG_WEBP)) {
                        $oImage = @imagecreatefromwebp($sourceFile);
                    } else {
                        // WebP not supported by PHP GD, will fallback to copy original
                        return false;
                    }
                }
                break;
        ...
        if ($sourceImageAttr['mime'] == 'image/png' || $sourceImageAttr['mime'] == 'image/webp')
        {
        ...
        switch ($sourceImageAttr['mime'])
        {
            ...
            case 'image/webp':
                if (function_exists('imagewebp')) {
                    imagewebp($oThumbImage, $targetFile, $quality);
                } else {
                    // Fallback to PNG if WebP save not supported
                    imagepng($oThumbImage, $targetFile);
                }
                break;
            case 'image/wbmp':
