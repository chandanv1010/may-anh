...
function CheckAuthentication()
{
    ...
    if(!isset($_COOKIE['CKFINDER_USER']) || empty($_COOKIE['CKFINDER_USER'])){
        return false;
    }
...
$username = $_COOKIE['CKFINDER_USER'] ?? null;
$permissions = [];
if(isset($_COOKIE['CKFINDER_PERMISSION'])){
    $permissions = json_decode(base64_decode($_COOKIE['CKFINDER_PERMISSION']), true);
...
function hasPerm($perm, $permissions){
    return in_array($perm, $permissions);
}



$hasAllPermission = hasPerm('ckfinder:all', $permissions);
...
ATTENTION: The trailing slash is required.
*/
$baseDir = str_replace('\\', '/', (dirname(dirname(dirname(dirname(__FILE__)))).'/public/userfiles/'));


if(!$hasAllPermission && $username){
    $userDir = $baseDir . 'users/' . $username . '/';
...
$config['Thumbnails'] = Array(
	...
	'enabled' => true, // Enable thumbnails để hiển thị preview hình ảnh
	'directAccess' => false,
	'maxWidth' => 150,
	'maxHeight' => 150,
	'bmpSupported' => false,
...
$config['AccessControl'][] = Array(
		...
		'folderView' => $hasAllPermission || hasPerm('ckfinder:folderView', $permissions),
		'folderCreate' => $hasAllPermission || hasPerm('ckfinder:folderCreate', $permissions),
		'folderRename' => $hasAllPermission || hasPerm('ckfinder:folderRename', $permissions),
		'folderDelete' => $hasAllPermission || hasPerm('ckfinder:folderDelete', $permissions),
		'fileView' =>  $hasAllPermission || hasPerm('ckfinder:fileView', $permissions),
		'fileUpload' =>  $hasAllPermission || hasPerm('ckfinder:fileUpload', $permissions),
		'fileRename' =>  $hasAllPermission || hasPerm('ckfinder:fileRename', $permissions),
		'fileDelete' =>  $hasAllPermission || hasPerm('ckfinder:fileDelete', $permissions),
...
if(!$hasAllPermission && $username){
    $config['ResourceType'][0]['directory'] = $userDir . 'file/';
    $config['ResourceType'][0]['url'] = $userUrl . 'file/';

    $config['ResourceType'][1]['directory'] = $userDir . 'image/';
    $config['ResourceType'][1]['url'] = $userUrl . 'image/';

    $config['ResourceType'][2]['directory'] = $userDir . 'flash/';
    $config['ResourceType'][2]['url'] = $userUrl . 'flash/';

    if(!file_exists($config['ResourceType'][0]['directory'])){
        mkdir($config['ResourceType'][0]['directory'], 0755, true);
    }

    if(!file_exists($config['ResourceType'][1]['directory'])){
        mkdir($config['ResourceType'][1]['directory'], 0755, true);
    }

    if(!file_exists($config['ResourceType'][2]['directory'])){
        mkdir($config['ResourceType'][2]['directory'], 0755, true);
    }
}
